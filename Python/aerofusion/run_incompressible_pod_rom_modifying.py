...
##
    ...
    if options.pod.pod_rom_1basis: 
        print('Calculating', options.pod.num_velocity_modes, \
               'POD modes of velocity in 1basis approach')
        (phi, modal_coeff, pod_lambda) = pod_modes.Find_Modes(\
          mean_reduced_velocity,
            weights_ND,
              options.pod.num_velocity_modes)
    ...
    if options.pod.pod_rom_2basis: 
        print('Calculating', options.pod.num_velocity_modes, \
               'POD modes of velocity in 2basis approach')
        (phi, modal_coeff_phi, pod_lambd_phi) = pod_modes.Find_Modes(\
          mean_reduced_velocity,
            weights_ND,
              options.pod.num_velocity_modes)
        (psi, modal_coeff_psi, pod_lambda_psi) = pod_modes.Find_Modes(\
          mean_reduced_grad_velocity,
            weights_3ND,
              options.pod.num_grad_velocity_modes)
        print('phi, lambda', phi.shape, pod_lambda_phi.shape)
        print('psi, lambda', psi.shape, pod_lambda_psi.shape)
        print('Saving POD data to file', pod_data_filename)
        np.savez(pod_data_filename,
              simulation_time = simulation_time,
              phi = phi,
