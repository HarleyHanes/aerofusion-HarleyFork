...
def main(poi, poi_selector, qoi_selector,un_normalizer):
    #================================Load POI Values===========================
    ...
    basis_location_mat = np.empty((n_samp,0))
    basis_extent_mat = np.empty((n_samp,0))
    #Load pois and unnormalize
    if n_samp ==1:
        for i_poi in range(n_poi):
            if poi_selector[n_poi].lower() == 're':
                reynolds_number_vec = un_normalizer(poi[[i_poi]], 're')
            elif poi_selector[n_poi].lower() == 'boundary exponent':
                boundary_exp_vec = un_normalizer(poi[[i_poi]], 'boundary exponent')
            elif poi_selector[n_poi].lower() == 'penalty strength':
                pen_strength_vec = un_normalizer(poi[[i_poi]], 'penalty strength')
            elif poi_selector[n_poi].lower()[0:10] == 'basis vort':
                basis_vort_mat=np.append(basis_vort_mat, \
                                         un_normalizer(poi[i_poi], 'basis vort').reshape(1,1),\
                                         axis=1)
            elif poi_selector[n_poi].lower()[0:12] == 'basis orient':
                basis_orient_mat=np.append(basis_orient_mat, \
                                           un_normalizer(poi[i_poi], 'basis orient').reshape(1,1), \
                                           axis=1)
            elif poi_selector[n_poi].lower()[0:14] == 'basis location':
                basis_location_mat=np.append(basis_location_mat,\
                                             un_normalizer(poi[i_poi], 'basis location').reshape(1,1), \
                                             axis=1)
            elif poi_selector[n_poi].lower()[0:12] == 'basis extent':
                basis_extent_mat=np.append(basis_extent_mat, \
                                           un_normalizer(poi[i_poi], 'basis extent').reshape(1,1), \
                                           axis=1)
            else :
        ...
        for i_poi in range(n_poi):
            if poi_selector[n_poi].lower() == 're':
                ...
                basis_vort_mat=np.append(basis_vort_mat, \
                                         un_normalizer(poi[:,i_poi], 'basis vort').reshape(n_samp,1),\
                                         axis=1)
            elif poi_selector[n_poi].lower()[0:12] == 'basis orient':
                basis_orient_mat=np.append(basis_orient_mat, \
                                           un_normalizer(poi[:,i_poi], 'basis orient').reshape(n_samp,1), \
                                           axis=1)
            elif poi_selector[n_poi].lower()[0:14] == 'basis location':
                basis_location_mat=np.append(basis_location_mat,\
                                             un_normalizer(poi[:,i_poi], 'basis location').reshape(n_samp,1), \
                                             axis=1)
            elif poi_selector[n_poi].lower()[0:12] == 'basis extent':
                basis_extent_mat=np.append(basis_extent_mat, \
                                           un_normalizer(poi[:,i_poi], 'basis extent').reshape(n_samp,1), \
                                           axis=1)
            else :
                raise Exception("Unrecognized poi name" + str(poi_selector[i_poi]))
    for i_samp in range(n_samp):
        ...
        basis_location_vec = basis_location_mat[i_samp]
        basis_extent_vec = basis_extent_mat[i_samp]
        #Construct snapshot and pod
        if i_samp >= 1:
            basis_vort_vec = basis_vort_mat[i_samp-1]
            basis_orient_vec = basis_orient_mat[i_samp-1]
            basis_location_vec = basis_location_mat[i_samp-1]
            basis_extent_vec = basis_extent_mat[i_samp-1]
            ...
            if np.all(basis_vort_vec == basis_vort_vec_old) and \
               ...
               np.all(basis_location_vec == basis_location_vec_old) and \
               np.all(basis_extent_vec ==basis_extent_vec_old):
                   #Load old pod
            else:
                mean_reduction = get_artificial_mean(basis_vort_vec, basis_orient_vec,\
                                                     basis_location_vec, basis_extent_vec)
